...
OSPF routing protokolünde, ağ topolojisini öğrenmek ve en iyi rotaları hesaplmak için LSA(Link State Advertisement) adı verilen paketleri kullanır.
Her LSA tipi farklı bir amaca hizmet eder ve farklı bir kapsama alanına sahiptir 

OSPF veritabınında (LSDB - Link State Database) depolanmaktadır.
...
- LSDB deki bu kısım, area içindeki komşu olan tüm routerları gösterir. 
...
Bu kısım ortamdaki DR Router'ın interface ip adresi ve Router ID'sini gösterir. Bu LSA 2 DR router tarafından üretilir.
...
Farklı bir örneğe baktığımızda burada 2 adet DR router gözüküyor. 
...
Bu kısımd ABR router tarafından üretilen farklı arealardaki rota bilgilerini taşıyan LSA paketleridir. 
...
- ADV Router    Link id deki networkü duyuran router. Bu Routera ABR router denilir.
...
Bu kısım OSPF topolojisinde EXternal rotaları duyuran ASBR router nerede sorusunun cevabıdır. <br>
...
Bu kısım external rotaları duyuruan paketlerdir. 
...
- Link       Duyurulan External rotanın network idsi
